...
class PageCacheFilterTest extends TestCase
{
    ...
    public function testOnReadyServesCachedBodyAndStopsFlow(): void
    {
        ...
        $request->expects($this->once())
            ->method('all')
            ->willReturn(['q' => 'ok', 'ignored' => '/ignored', 'empty' => '']);
        $request->method('header')
            ->willReturnCallback(
                static fn(string $name, ?string $default = null): ?string => match ($name) {
        ...
        $redis->expects($this->once())
            ->method('hGet')
            ->with(
                'cache:test:' . PageCacheFilterControllerStub::class . '::withDefaultKey:ignored=%2Fignored&q=ok:json',
                'etag'
            )
            ->willReturn('etag-cache');
        $redis->expects($this->once())
            ->method('hGetAll')
            ->with('cache:test:' . PageCacheFilterControllerStub::class . '::withDefaultKey:ignored=%2Fignored&q=ok:json')
            ->willReturn([
                'etag' => 'etag-cache',
